...
/**
 * Implementação genérica de um {@link Comparator} para comparações simples entre campos.
 * 
...
public class GenericComparator implements Comparator<Object> {
	 ...
	 */
	public GenericComparator(boolean asc, String... properties) {
		if (properties != null) {
	 ...
	 */
	public GenericComparator(List<PropertyConfig> configs) {
	...
	public static void sort(boolean ascendent, List<?> list, String...properties) {
		sort(list, new GenericComparator(ascendent, properties));
	}
	...
	public static void sort(List<?> list, PropertyConfig...properties) {
		sort(list, new GenericComparator(Arrays.asList(properties)));
	}
	
	private static void sort(List<?> list, GenericComparator comparator) {
		if (list != null) {
	...
	public int compare(Object o1, Object o2) {
		try {
			...
			throw new RuntimeException("Falha ao executar comparador genérico", e);
		}
	}
